...
func init() {
	listCmd.AddCommand(listProjectPipelineJobsCmd)

	listProjectPipelineJobsCmd.Flags().StringVarP(&projectPipelineJobsScope, "scope", "s", "", "Scope")
	listProjectPipelineJobsCmd.Flags().BoolVar(&projectPipelineJobsPrettyOutput, "pretty", false, "Use custom output formatting")
}
...
var listProjectPipelineJobsCmd = &cobra.Command{
